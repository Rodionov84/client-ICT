      ...
      <table>
        <thead>
          <tr className="table-header-list-claims">
            <th onClick={() => setSelectionFormView("wrap-form")} className="table-first-sell" title="Выбрать сотрудника">
              &nbsp;&nbsp;{currentEmployee ? currentEmployee : "ФИО"}
            </th>
            <th onClick={() => setStatePeriod(null)} className="table-second-sell">
              Всего:
            </th>
            {periods.map(period => {
              return <th onClick={() => setStatePeriod(period.num)} className="th-table-header">
                <div className="th-img-block">
                  <svg id={period.num} viewBox="0 0 122.87 122.88" xmlns="http://www.w3.org/2000/svg" fillRule="currentColor">
            ...
            <th onClick={() => setStatePeriod(1095)} title={"более 1095 дней, возможно превышение срока исковой давности"} className="th-table-header-1095">
              <div className="th-img-block-1095">
                <svg xmlns="http://www.w3.org/2000/svg" fillRule="black" className="bi bi-hourglass-bottom" viewBox="0 0 16 16">
        ...
        <tbody>
          <tr className="table-row-responsible-level">
            <td className="arrow-cursor">&nbsp;&nbsp;&nbsp;Является ответственным по текущей стадии рассмотрения:</td>
            <td onClick={() => showCaimsPersonal(currentEmployee, null, null)} className="cells-with-numbers">
              {getSumPersonal(claims, null, currentEmployee, null)}
            </td>
            {periods.map(period => {
              return <td
                onClick={() => showCaimsPersonal(currentEmployee, null, period.num)}
                className="cells-with-numbers">
          ...
          <tr className="table-row-responsible-level">
            <td className="arrow-cursor">
              &nbsp;&nbsp;&nbsp;Является ответственным куратором убытка:
            </td>
            <td onClick={() => showCaimsPersonal(null, currentEmployee, null)} className="cells-with-numbers">
        ...
        <tfoot className="footer-personal-list" >
          <tr>
            <td className="arrow-cursor">&nbsp;&nbsp;Итого:</td>
            <td onClick={() => showCaimsPersonalAll(currentEmployee, currentEmployee, null)} className="cells-with-numbers">
            ...
            {periods.map(period => {
              return <td onClick={() => showCaimsPersonalAll(currentEmployee, currentEmployee, period.num)}
                className="cells-with-numbers">{getSumPersonal(claims, period.num, currentEmployee, currentEmployee)}</td>
            })}
            <td onClick={() => showCaimsPersonalAll(currentEmployee, currentEmployee, 21)}
              className="cells-with-numbers">{getSumPersonal(claims, 21, currentEmployee, currentEmployee)}</td>
            <td onClick={() => showCaimsPersonalAll(currentEmployee, currentEmployee, 1095)}
              className="cells-with-numbers">{getSumPersonal(claims, 1095, currentEmployee, currentEmployee)}</td>
          </tr>
        </tfoot>
      </table>
    </div>
